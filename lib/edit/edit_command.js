...
module.exports = async (section, action) => {
  ...
  if (program.helpOption) return program.customHelpOption()

  if (!program.instance) {
    return errors_.exitMessage('missing instance', instanceHint)
  }

  let { args, batch, exitOnError = true } = program
  if (batch) {
...
const runEditCommand = async (section, action, args) => {
  ...
  await assertCredentials(config)
...
const runInBatch = (section, action, exitOnError) => {
  ...
  .on('data', async function (line) {
    try {
      ...
      if (line === '') return
      this.pause()
      let errLog = errors > 0 ? ` (errors in previous lines: ${errors})` : ''
